 ...
 */
internal interface PlayerErrorHandler : ErrorHandler<PlayerFailures> {
    class Base @Inject constructor() : PlayerErrorHandler {
        override fun handle(throwable: Throwable) = when (throwable) {
            else -> PlayerFailures.OtherError(throwable)
        }
    }
}
