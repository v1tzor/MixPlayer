...
internal fun ThemeChooserDialog(
    modifier: Modifier = Modifier,
    initTheme: ThemeUiType,
    onCloseDialog: () -> Unit,
    onChooseTheme: (ThemeUiType) -> Unit,
) {
    var currentTheme by remember { mutableStateOf(initTheme) }
        ...
        Surface(
            ...
            Column {
                Box(modifier = Modifier.padding(24.dp)) {
                    Text(
                        text = SettingsThemeRes.strings.themeDialogTitle,
                        color = MaterialTheme.colorScheme.onSurface,
                        style = MaterialTheme.typography.headlineSmall,
                    )
                }
                LazyColumn(modifier = Modifier.height(300.dp)) {
                    items(ThemeUiType.values()) { theme ->
                        SettingsDialogItem(
                            modifier = Modifier.fillMaxWidth(),
                            selected = theme == currentTheme,
                            title = theme.mapToMessage(),
                            onSelectChange = { currentTheme = theme },
                        )
                    }
                }
                DialogButtons(
                    onCancelClick = onCloseDialog,
                    onConfirmClick = { onChooseTheme(currentTheme) },
                )
            }
        }
    }
